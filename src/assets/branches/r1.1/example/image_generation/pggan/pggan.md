...
![4x4](assets/branches/r1.1/example/image_generation/pggan/Figure/pggan_4x4.png)
...
![8x8](assets/branches/r1.1/example/image_generation/pggan/Figure/pggan_8x8.png)
Both the generator and the discriminator progressively grow until reaching the desired resolution of $1024\times 1024$.
![1024x1024](assets/branches/r1.1/example/image_generation/pggan/Figure/pggan_1024x1024.png)
...
![grow](assets/branches/r1.1/example/image_generation/pggan/Figure/pggan_smooth_grow.png)
...
![png](assets/branches/r1.1/example/image_generation/pggan/pggan_files/pggan_10_0.png)
...
class Gen(torch.nn.Module):
    ...
    def forward(self, x):
        for g in self.g_blocks[:-1]:
            x = g(x)
        previous_img = self.rgb_blocks[0](assets/branches/r1.1/example/image_generation/pggan/x)
        previous_img = self.upsample(previous_img)
        x = self.g_blocks[-1](assets/branches/r1.1/example/image_generation/pggan/x)
        new_img = self.rgb_blocks[1](assets/branches/r1.1/example/image_generation/pggan/x)
...
class Disc(torch.nn.Module):
    ...
    def forward(self, x):
        new_x = self.rgb_blocks[1](assets/branches/r1.1/example/image_generation/pggan/x)
        new_x = self.d_blocks[-1](assets/branches/r1.1/example/image_generation/pggan/new_x)
        downscale_x = self.pool(x)
        downscale_x = self.rgb_blocks[0](assets/branches/r1.1/example/image_generation/pggan/downscale_x)
